...
module.exports = function (req, res, next) {
  ...
  console.log("procssing admin command for user ", params.user_id)
  isAdmin(params.user_id, params, req, res, execute, deny, next)
}

function isAdmin(user_id, params, req, res, success, failure, next) {
  const client = storage()
  return client.hget("admins", user_id).then(function(is_admin) {
    if (is_admin == "true") {
